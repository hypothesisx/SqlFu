...
namespace SqlFu
{
    public static class CrudHelpers
    {
      ...
      /// <param name="schemaName">Optional schema name that is prepended to the table name.</param>
      /// <returns></returns>
      public static LastInsertId Insert(this DbConnection db, string table, object data, bool idIsIdentity = true, string schemaName = null)
        {
            table.MustNotBeEmpty();
            data.MustNotBeNull();
            var ti = new TableInfo(table);
            ti.AutoGenerated = idIsIdentity;
            return Insert(db, ti, data, schemaName);
        }

        public static LastInsertId Insert<T>(this DbConnection db, T data, string schemaName = null) where T : class
        {
            data.MustNotBeNull();
            return Insert(db, TableInfo.ForType(typeof (T)), data, schemaName);
        }


        private static LastInsertId Insert(DbConnection db, TableInfo tableInfo, object data, string schemaName = null)
        {
            ...
            if (tableInfo.InsertSql == null)
            {
                ...
                sb.AppendFormat("{0}.{1} (", (schemaName??"dbo"), provider.EscapeName(tableInfo.Name));
        ...
        /// <param name="schemaName">Optional schema name that is prepended to the table name.</param>
        /// <returns></returns>
        public static int Update(this DbConnection db, string table, object data, object id = null, string schemaName = null)
        {
            var ti = new TableInfo(table);
            return Update(db, ti, data, id, schemaName);
        }

        /// <summary>
        /// If both poco has id property and the Id arg is specified, the arg is used
        /// </summary>
        public static int Update<T>(this DbConnection db, object data, object id = null, string schemaName = null)
        {
        ...
        public static int UpdateWhereColumn(this DbConnection db, string tableName, object data, string colName,
                                            object columnValue, string schemaName = null)
        {
        ...
        public static int Update<T>(this DbConnection db, object data, Expression<Func<T, bool>> criteria, string schemaName = null)
        {
        ...
        /// <param name="schemaName">Optional schema name that is prepended to the table name.</param>
        /// <returns></returns>
        public static IBuildUpdateTable<T> Update<T>(this DbConnection db, string schemaName = null)
        {
            return new UpdateTableBuilder<T>(db, schemaName);
        }

        private static int Update(DbConnection db, TableInfo ti, object data, object id = null,
                                  string filterColumn = null, string schemaName = null)
        {
            var sb = new StringBuilder();
            sb.AppendFormat("update {0}.{1} set", (schemaName??"dbo"), db.EscapeIdentifier(ti.Name));
      ...
      {
        return DeleteFrom<T>(db, condition, null, args);
      }
        public static int DeleteFrom<T>(this DbConnection db, string condition, string schemaName, params object[] args)
        {
            var ti = TableInfo.ForType(typeof (T));
            return
                db.Execute(
                    string.Format("delete from {0}.{1} where {2}", (schemaName??"dbo"), db.GetProvider().EscapeName(ti.Name), condition), args);
        }
